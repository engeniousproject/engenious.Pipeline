...
namespace engenious.Content.Pipeline
{
    public class TextureContent
    {
        ...
        private void CreateTexture(GraphicsDevice graphicsDevice, bool generateMipMaps, int mipMapCount, IntPtr inputData, int width, int height, TextureContentFormat inputFormat, TextureContentFormat outputFormat)
        {
            ...
            GL.TexImage2D(TextureTarget.Texture2D, 0, (hwCompressedOutput ? (OpenToolkit.Graphics.OpenGL.PixelInternalFormat)outputFormat : OpenToolkit.Graphics.OpenGL.PixelInternalFormat.Rgba), width, height, 0, (hwCompressedInput ? (OpenToolkit.Graphics.OpenGL.PixelFormat)inputFormat : OpenToolkit.Graphics.OpenGL.PixelFormat.Bgra), PixelType.UnsignedByte, inputData);
            if (doGenerate)
            {
        ...
        private void PreprocessMipMaps(GraphicsDevice graphicsDevice)
        {
            ...
            for (int i = 0; i < (GenerateMipMaps ? 1 : MipMapCount); i++)
            {
                ...
                else
                {
                    ...
                    GL.BindTexture(TextureTarget.Texture2D,_texture);
                    GL.GetTexImage(TextureTarget.Texture2D,i,OpenToolkit.Graphics.OpenGL.PixelFormat.Bgra,PixelType.UnsignedByte,bmpData.Scan0);
